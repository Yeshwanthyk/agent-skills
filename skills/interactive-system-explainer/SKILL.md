...
5. **Write one self-contained `.html`** — inline HTML/CSS/JS, no bundler, no npm, no CDN unless approved. Put all CSS in a `<style>` block inside `<head>` before body content. Open directly in a browser.
...
<head>
  metadata
  <style> all CSS, including theme tokens and overflow guards
</head>
<body>
