# Long-form Documents Playbook (Notion adapter example)
...
Long-form documentation is where rationale often lives before it becomes code: PRDs, specs, RFCs, ADRs, meeting notes, runbooks, and postmortems. Notion is one adapter example.

## How to search it

Use the document-search and document-fetch tools exposed by the current harness. If the adapter is Notion, use the operations below; translate the same sequence for Confluence, Google Docs, Coda, or another available document system. Inspect tool schemas before calling them.
