# Issue / Ticket Tracker Playbook (Linear adapter example)
...
Issue trackers are where product and business context often lives: customer requests, compliance deadlines, initiative framing, scope changes, and incident follow-up. Linear is one adapter example.

## How to search it

Use the issue-tracker tools exposed by the current harness. If the adapter is Linear, use the operations below; translate the same sequence for Jira, GitHub Issues, Plane, Shortcut, or another available tracker. Inspect tool schemas before calling them.
