# Source Control Archaeology (git + hosted review adapter)

## What this source contains

- Commit history (messages, dates, authors, diffs)
- PR descriptions, review comments, and discussion threads (via an available hosted-repository adapter, such as `gh`)
...
Source control is closest to the shipped change, but squash merges, rewritten history, inaccessible reviews, and undocumented decisions make it incomplete.
...
For each substantive commit, pull the hosted review context when an adapter is available. Use the current harness's hosted-repository tool for the full PR or review record; the `gh pr view` command below is only an optional GitHub CLI equivalent.
