# Infrastructure Observability Playbook (Datadog adapter example)

## What this source contains

Infrastructure observability holds the runtime record: what happened in production, as opposed to what was planned or discussed. Datadog is one adapter example.

- **Metrics.** Counters, gauges, and histograms. Their presence is circumstantial evidence that someone measured the value, not why the target code exists.
- **Monitors & alerts.** Watched conditions and thresholds. They are circumstantial unless their text or linked record explicitly connects them to the target decision.
- **Dashboards.** Curated views that show one operational framing. Treat relevance as circumstantial unless an annotation or linked record states the rationale.
...
Observability answers "what was the production reality around the time this code was written?", which can explain the code's shape.

## How to search it

Use the observability tools exposed by the current harness. If the adapter is Datadog, use the operations below; translate the same sequence for New Relic, Honeycomb, Grafana, Splunk, or another available system. Start broad, then narrow.
...
2. **Dashboards and monitors first. They tell you what the team cares about.**
   ...
   When a dashboard or monitor covers the target, note its queries and watched thresholds. A matching threshold is a lead; it becomes direct evidence only through an explicit textual link to the target decision.
...
3. **Metrics around the target.**
   ...
   Correlating a metric's trajectory with the target's add/change date is circumstantial evidence: "the `payment_timeout` metric spiked 2023-11-03, and the retry logic merged 2023-11-06." Check neighboring changes and seek a record that links the events.
...
- A monitor whose description or linked incident explicitly connects its threshold to the constraint the code enforces
- A dashboard annotation or linked record that names the target decision
- A metric showing a production spike immediately before the code was merged, reported as circumstantial unless another source states the connection
