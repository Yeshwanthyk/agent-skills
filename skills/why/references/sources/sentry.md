# Error / Exception Tracking Playbook (Sentry adapter example)

## What this source contains

Error tracking is the archive of things that went wrong. For defensive, corrective, or error-handling code, it can hold direct motivation: specific exceptions, stack traces, and frequencies that preceded a check, catch, retry, or fallback. Sentry is one adapter example.
...
The most valuable signal is **temporal correlation**: an issue appears, peaks, and stops around the release that shipped the target change. Correlation remains circumstantial unless the record links the issue to the change.

## How to search it

Use the error-tracking tools exposed by the current harness. If the adapter is Sentry, use the operations below; translate the same sequence for Rollbar, Bugsnag, Airbrake, or another available system. Inspect tool schemas first.
