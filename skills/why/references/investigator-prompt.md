...
Build one prompt from this template per evidence category. Fill in the placeholders, then append the single category playbook `sources/<source>.md` matching the investigator's category (see `source-playbook.md`). Adapt the playbook to the evidence tools exposed by the current harness. If the target code looks defensive (null checks, retry logic, timeout handling, rate limiting, feature flags, egress guards, OOM handlers), also append `sources/incident-postmortem.md` for the incident-flavored queries in that category.

---

You are investigating the historical context and motivation behind a piece of code. A separate synthesizer combines your findings with other investigators' into a final answer, so gather evidence accurately rather than writing a polished narrative.
...
**Available evidence tools and scope:** {TOOLS_AND_SCOPE}
...
3. **Follow links within your assigned source.** If a PR references another PR or commit, pull it. If a ticket links a parent or sibling, pull it. If a doc links another doc, pull it. Stay inside your assigned category. When you spot a cross-category reference, record it under "Additional Leads" for that category's investigator rather than chasing it yourself. The one-investigator-per-category design depends on this; chasing cross-category links duplicates work and confuses scope.
...
6. **Watch for contradictions.** If two items in your category disagree, record both. Don't suppress the inconvenient one.
...
### Source
Which of the seven categories you investigated and which available tools you used. Treat repository code comments as source-control evidence, not a separate category.

### What I Searched
The exact queries, bounded time windows, items opened, tool calls, and places examined. Be specific. This tells the synthesizer how thorough the investigation was and what might still be unsearched.
...
### Gaps
What you searched for and didn't find, plus access, retention, schema, or sampling limits. Be specific: "Searched the issue tracker for [query] across [time range]. No matching issues." These absences are valuable data. Distinguish a null result from an unavailable tool.
