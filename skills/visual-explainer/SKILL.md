...
**When to defer to `interactive-system-explainer` instead.** This skill is for single-page artefacts: one diagram, one comparison, one dashboard, one slide deck. When the user asks for an interactive system walkthrough that needs ≥6 distinct concepts (boundary, models, state machine, sequence, history graph, algorithm, scenarios, deploy, comparison, source map) or a real **scenario simulator** with state mutation and event log — reach for `interactive-system-explainer`. That skill specialises in tab-navigated multi-section explainers with reusable interactive components (sequence player, DAG, token meter, cut-point timeline, scenario engine, invariants, citations) and ships a four-step validation ritual. The flue v2 explainer (~/Downloads/flue-explainer-v2.html) is the canonical example.
