...
1. Capture a baseline trace through the matching project verifier or current harness UI, browser, CLI, API, or profiling tools. If no route can capture the real workload, stop with the missing capability instead of inventing a control skill.
...
3. Plan the fix from the trace. If it crosses a function boundary, use `architect` first. Delegate implementation through the current managed runtime when that reduces context load, then review the diff. Capture a post-fix trace.
   Apply [`sequence-verifiable-units`](../principles/sequence-verifiable-units.md), verifying each attempt before trying the next.
...
The baseline and post-change measurements use the same workload, the result is proven on the real surface, and the artifact path is recorded.
