...
1. **Session history.** Load the shared [session-records contract](../yesh-mode/references/session-records.md). Locate the active project's saved sessions, then read the header, active branch, and relevant compaction or branch-summary entries.
...
Use a managed worker runtime for parallel history mining only when the corpus warrants it. Load and follow the shared [agent-routing contract](../yesh-mode/references/agent-routing.md). If delegation is unavailable, continue locally and report the gap.
